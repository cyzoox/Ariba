...
export default class SearchRentalsProperty extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      dataSource: this.props.route.params.storesList,
      loading: false,
      data: [],
      error: null,
      items: [],
  ...
  searchFilterFunction = async () => {
    ...
    const result = this.props.route.params.storesList
      .filter(items => {
    ...
    const paidResult = this.props.route.params.storesList
      .filter(items => {
        const itemData = items.name.toUpperCase();
        const textData = this.state.searchText.toUpperCase();
...
<TouchableOpacity
        ...
        this.state.selectedFilterBy == 'ByProductSearch' ? (
          ...
          <FlatList
            ...
            renderItem={({item}) => (
              <Box transparent>
                <Box style={{flex: 1, marginHorizontal: 20}}>
                  <TouchableHighlight
